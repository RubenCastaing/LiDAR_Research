...
# Function to visualize a single file
# ... (your existing function code here, just return 'pcd' and 'mesh')
...
# Initialize a point cloud and mesh as empty, to update later
pcd = o3d.geometry.PointCloud()
mesh = o3d.geometry.TriangleMesh()

# Add initial point cloud and mesh to visualizer
vis.add_geometry(pcd)
# Uncomment below if you want to add mesh as well
# vis.add_geometry(mesh)

# Loop through all the files and visualize them one by one
for filename in file_names[:3]:  # Adjust this slice as needed
    full_path = os.path.join(folder_path, filename)
    print(f"Visualizing file: {filename}")

    # Call the function and get the processed point cloud and mesh
    new_pcd, new_mesh = visualize_single_file(full_path)

    print(new_pcd)
    print(new_mesh)

    # Update point cloud and mesh data
    pcd.points = new_pcd.points
    pcd.colors = new_pcd.colors
    pcd.normals = new_pcd.normals
    # Uncomment below if you want to update mesh as well
    ...
    #view_control.set_constant_z_far(1000)  # Adjust as needed
    vis.poll_events()
    vis.update_renderer()
    
    time.sleep(1)  # Pause for a while (optional)
